...
import machine_parallel_loss_splitnum
...
def _eval_data(eval_datas, eval_metric, part_size, executor_manager, eval_batch_end_callback, epoch, logger):
    ...
    for k, _eval_data in enumerate(eval_datas):
        ...
        for name, value in name_value:
            logger.info('Epoch[%d] eval data-%d Validation-%s=%f', epoch, k, name, value)  #Yu
...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
                        ...
                        eval_end_callback=None, eval_first=False,
    ...
    if logger is None:
        logger = logging
    trainable_param_names = [key for key in param_names if key not in fixed_param_names]
    logging.info("trainable params: {}".format(trainable_param_names))
    ...
    executor_manager = DataParallelExecutorManager(symbol=symbol,
                                                   ...
                                                   param_names=trainable_param_names,
                                                   arg_names=arg_names,
                                                   aux_names=aux_names,
    ...
    device_part_size = num_classes / num_workers
    device_slices = [slice(i*device_part_size, (i+1)*device_part_size) for i in range(num_workers)]
    if device_part_size*num_workers is not num_classes:
        device_slices[-1] = slice((num_workers-1) * device_part_size, num_classes)
    device_part_shape = []
    ...
    updater_fc = get_updater(optimizer_fc)

    softmax_executor = machine_parallel_loss_splitnum.SoftmaxModelParallelOperator(train_data.batch_size*num_workers/split_parts,
                                                                          feature_shape,
                                                                          ctx, comm, device_part_size,
                                                                          updater_fc, no_bias, margin, easy_margin,
                                                                          margin_s, margin_m)
    ...
    eval_datas = eval_data if isinstance(eval_data, list) else [] if eval_data is None else [eval_data]

    if eval_first:
        _eval_data(eval_datas, eval_metric, slice_batch_size, executor_manager, eval_batch_end_callback, begin_epoch - 1,
                   logger)

    logging.info('rank:%d Start Epoch'%(kvstore.rank))
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                for index,islice in enumerate(split_slices):
                    # forward to get feature
                    #logging.info('Slice %d loading data'%(index))
                    ...
                    #logging.info('executor done %d, rank:%d'%(index,kvstore.rank))
                    ...
                    #logging.info('mpi_allgather done %d, rank:%d'%(index,kvstore.rank))
                    ...
                    #logging.info('softmax forward done %d, rank:%d'%(index,kvstore.rank))

                    softmax_executor.backward(max_exp, sum_exp)

                    #logging.info('softmax executor done %d, rank:%d'%(index,kvstore.rank))
                    ...
                    if update_on_kvstore:
                        total_weight = nd.empty(fullyconnect_params['weight'].shape)
                        total_bias = nd.empty(fullyconnect_params['bias'].shape)
                        mpi_allgather(weight_tmp, total_weight)
                        mpi_allgather(bias_tmp, total_bias)
                        total_weight_before = total_weight.asnumpy()
                        total_weight.attach_grad()
                        total_feature.attach_grad()
                        with autograd.record():
                        #out_1 = nd.FullyConnected(data=total_feature, weight=total_weight, bias=total_bias, num_hidden=num_classes)
                        #out_2 = nd.SoftmaxOutput(data=out_1, label=total_label)
                            out_2 = test_func(total_weight, total_label, total_feature, num_classes, easy_margin, margin_s, margin_m)
                        out_2.backward()
                        out_grad_test = total_feature.grad
                        weight_grad_test = total_weight.grad
                        print(out_grad_test)
                        updater_fc(11111111, weight_grad_test, total_weight)
                        total_weight_update = total_weight.asnumpy() - total_weight_before
                    else:
                        cpu_out_feature.attach_grad()
                        with autograd.record():
                        #out_1 = nd.FullyConnected(data=cpu_out_feature, weight=weight_tmp, bias=bias_tmp, num_hidden=num_classes)
                        #out_2 = nd.SoftmaxOutput(data=out_1, label=data_batch.label[0])
                            out_2 = test_func(cpu_out_feature, data_batch.label[0], True, 1, 0)
                        out_2.backward()
                        out_grad_test = cpu_out_feature.grad
                        print(out_grad_test)

                    # backward
                    executor_manager.backward(out_grad)

                    #logging.info('backward all done %d'%(index))
                    ...
                    if nbatch % 100 == 0:
                        logging.info('softmaxloss in batch %d: %f'%(nbatch,batch_loss.mean().asnumpy()))
                ...
                w0 = softmax_executor.local_name['part_weight_0']
                w1 = softmax_executor.local_name['part_weight_1']
                w2 = softmax_executor.local_name['part_weight_2']
                w3 = softmax_executor.local_name['part_weight_3']
                mp_total_weight = np.concatenate((w0.asnumpy(), w1.asnumpy(), w2.asnumpy(), w3.asnumpy()), axis=0)
                mp_total_weight_update = mp_total_weight - total_weight_before
                weight_update_diff = mp_total_weight_update - total_weight_update
                weight_update_diff_abs = np.absolute(weight_update_diff)
                weight_update_abs = np.absolute(total_weight_update)
                weight_update_rel_error = np.true_divide(weight_update_diff_abs, weight_update_abs)
                weight_abs_flat = total_weight_before.flatten()
                total_weight_update_flat = total_weight_update.flatten()
                weight_update_rel_error_flat = weight_update_rel_error.flatten()
                sort_update_rel_error = np.sort(weight_update_rel_error_flat)
                ind = np.unravel_index(np.argsort(weight_update_rel_error_flat, axis=None), weight_update_rel_error_flat.shape)
                ...
                if update_on_kvstore:
                    ...
                    #softmax_executor.update_metric(train_metric, total_predict)
                else:
                    train_metric.update(predict_index, data_batch.label[0])
                    #softmax_executor.update_metric(train_metric, predict_index)
                ...
                if batch_end_callback is not None:
                    batch_end_params = BatchEndParam(epoch=epoch,
                                                     nbatch=nbatch,
                                                     eval_metric=train_metric,  #LossValueMetric(),  #eval_metric,
                                                     locals=locals())
                    _multiple_callbacks(batch_end_callback, batch_end_params)

                # this epoch is done possibly earlier
                if epoch_size > 0 and nbatch >= epoch_size:
                    do_reset = False
                    break
            ...
            # this epoch is done
            if epoch_size < 0 or nbatch >= epoch_size:
                break
        ...
        if epoch_end_callback or epoch + 1 == end_epoch:
            ...
            softmax_executor.dumps(weight_tmp, bias_tmp)
            if update_on_kvstore:
                mpi_allgather(weight_tmp, fullyconnect_params['weight'])
                mpi_allgather(bias_tmp, fullyconnect_params['bias'])
            else:
        ...
        if eval_data:
            ...
            for i, eval_batch in enumerate(eval_data):
                out_shape = tuple([eval_data.batch_size] + list(executor_manager.output_shapes[0][1:]))
                cpu_out_feature = nd.zeros(out_shape)
                executor_manager.load_data_batch(eval_batch)
                executor_manager.forward(is_train=False)
                executor_manager.output_copyto(cpu_out_feature)
                ...
                if update_on_kvstore:
                    total_feature = nd.empty((eval_data.batch_size*num_workers,)+feature_shape)
                    total_label = nd.empty((train_data.batch_size*num_workers,))
                    mpi_allgather(cpu_out_feature, total_feature)
                    mpi_allgather(eval_batch.label[0], total_label)
                    softmax_executor.load_data(total_feature, total_label)
                else:
                    softmax_executor.load_data(cpu_out_feature, eval_batch.label[0])

                # fully connected and softmax
                max_exp = nd.empty((eval_data.batch_size*num_workers,))
                sum_exp = nd.empty((eval_data.batch_size*num_workers,))
                softmax_executor.forward(max_exp, sum_exp)
                if update_on_kvstore:
                    mpi_gather_bcast(max_exp, sum_exp, max_exp, sum_exp)
                ...
                predict_index = nd.empty((batch_size*num_workers,))
                predict_value = nd.empty((batch_size*num_workers,))
                softmax_executor.output_predict(predict_value, predict_index)
                if update_on_kvstore:
                    predict_value_kv = nd.empty((num_workers, batch_size*num_workers))
                    predict_index_kv = nd.empty((num_workers, batch_size*num_workers))
                    mpi_allgather(predict_value, predict_value_kv)
                    ...
                    device_begin = nd.array([device_slices[i].start for i in range(len(device_slices))])
                    total_predict = device_begin[predict_max_index] + predict_index_kv[predict_max_index, nd.arange(batch_size*num_workers)]
                    softmax_executor.update_metric(eval_metric, total_predict)
                else:
                    softmax_executor.update_metric(eval_metric, predict_index)
        ...
        _eval_data(eval_datas, eval_metric, slice_batch_size, executor_manager, eval_batch_end_callback, epoch, logger)   #Yu
        logging.info('Now the lr is %f'%(optimizer.learning_rate))
...
class FeedForward(BASE_ESTIMATOR):
    ...
    def __init__(self, symbol, ctx=None,
                 num_epoch=None, epoch_size=None, optimizer='sgd', optimizer_fc='sgd',
                 initializer_bb=Uniform(0.01),
                 initializer_fc=Uniform(0.01),
        ...
        self.initializer_bb = initializer_bb
        self.initializer_fc = initializer_fc
    ...
    def _init_params(self, inputs, num_classes, embedding_size, overwrite=False):
        ...
        #logging.info('init backbone args')
        for k, v in arg_params.items():
            ...
            if self.arg_params and k in self.arg_params and (not overwrite):
                ...
                self.initializer_bb(k, v)

        #logging.info('init backbone auxs')
        ...
        #logging.info('init fullyconnect')
        # fully connect param
        fullyconnect_params = {}
        ...
        for k, v in fullyconnect_params.items():
            if self.fullyconnect_params:
                fullyconnect_params[k] = self.fullyconnect_params[k][:]
            else:
                self.initializer_fc(k, v)
    ...
    def fit(self, X, y=None, train_metric='acc', eval_data=None, eval_metric='acc',fixed_param_names=[],
            split_num=None,
            embedding_size=500, num_classes=10, no_bias=False,
            margin=True, easy_margin=True, margin_s=64, margin_m=0.5,
            epoch_end_callback=None, batch_end_callback=None, kvstore='local', logger=None,
            work_load_list=None, monitor=None, eval_end_callback=LogValidationMetricsCallback(),eval_first=False,
        ...
        """
        logging.info('rank %d: margin_s:%f margin_m:%f, easy_margin:%d'%(kvstore.rank,margin_s,margin_m,easy_margin))
        ...
        logging.info('params init done:%d'%(kvstore.rank))
        ...
        if self.optimizer_fc:
            if isinstance(self.optimizer_fc, str):
                batch_size = data.batch_size
                if kvstore and 'dist' in kvstore.type and not '_async' in kvstore.type:
                    batch_size *= kvstore.num_workers
                opt_fc = opt.create(self.optimizer_fc,
                                    rescale_grad=(1.0/batch_size),
                                    **(self.kwargs))
            elif isinstance(self.optimizer_fc, opt.Optimizer) or isinstance(self.optimizer_fc, list):
                opt_fc = self.optimizer_fc
        ...
        _train_multi_device(self.symbol, self.ctx, arg_names, param_names, aux_names,
                            ...
                            eval_end_callback=eval_end_callback,eval_first=eval_first,
    ...
    def create(symbol, X, y=None, ctx=None,
               ...
               initializer_bb=Uniform(0.01), initializer_fc=Uniform(0.01),
        ...
        model = FeedForward(symbol, ctx=ctx, num_epoch=num_epoch,
                            ...
                            optimizer=optimizer, initializer_bb=initializer_bb,
                            initializer_fc=initializer_fc,**kwargs)
