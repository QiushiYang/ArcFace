...
class SoftmaxModelParallelOperator(object):
    ...
    def margin_backward(self, in_data, margin_s, margin_m, out_grad):
        gt_index = self._get_gt_index()
        #logging.info('size of margin_tmp_space:%d'%(len(self.margin_tmp_space)))
        for i in range(self.num_parts):
            if (gt_index[i][0].size) != 0:
                #logging.info('margin_tmp_space part size: @%d:'%(i))
                #logging.info(self.margin_tmp_space[i].shape)
    ...
    def output_loss(self, batch_loss, islice):
        #logging.info(self.batch_loss.shape)
        #logging.info(batch_loss[islice].shape)
