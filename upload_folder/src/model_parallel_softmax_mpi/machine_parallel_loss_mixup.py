...
class SoftmaxModelParallelOperator(object):
    def __init__(self, batch_size, feature_shape, ctx, comm, device_part_size, updater,
        ...
        if self.use_mixup:
            self.batch_label = nd.empty((2, self.batch_size))
        else:
        ...
        if self.use_mixup:
            self.local_name['fully_connected_out_mix'] = []
        ...
        for i in range(self.num_parts):
            ...
            if self.use_mixup:
                self.local_name['part_fully_connected_out_mix_%s'%i] = nd.empty((self.batch_size, self.part_shape[i]), ctx=self.ctx[i])
                self.local_name['fully_connected_out_mix'].append(self.local_name['part_fully_connected_out_mix_%s'%i])
        ...
        if self.use_mixup:
            self.local_name['softmax_grad_mix'] = []
        for i in range(self.num_parts):
            ...
            if self.use_mixup:
                self.local_name['part_grad_mix_%s'%i] = nd.empty((self.batch_size, self.part_shape[i]), ctx=self.ctx[i])
                self.local_name['softmax_grad_mix'].append(self.local_name['part_grad_mix_%s'%i])
        ...
        if self.use_mixup:
            self.batch_loss_mix = nd.empty((self.batch_size,))
    ...
    def load_data(self, input_feature, batch_label):
       ...
       #        self.local_name['part_in_feature_%s'%i][:] = (nd.L2Normalization(self.local_name['part_in_feature_%s'%i])*self.margin_s)[:]
        ...
        if self.use_mixup:
            self.batch_label[:] = batch_label.T[:]
        else:
            batch_label.copyto(self.batch_label)

    def margin_forward(self, in_data, margin_s, margin_m, gt_label):
        gt_index = self._get_gt_index(gt_label)
        for i in range(self.num_parts):
    ...
    def margin_backward(self, in_data, margin_s, margin_m, out_grad, gt_label):
        gt_index = self._get_gt_index(gt_label)
        for i in range(self.num_parts):
            if (gt_index[i][0].size) != 0:
    ...
    def softmax_backward(self, in_data, total_sum, total_max, gt_label, loop=0):
        for i in range(self.num_parts):
        ...
        for i in range(self.num_parts):
            ...
            if (gt_index[i][0].size) != 0:
                ...
                if loop == 0:
                    self.batch_loss[gt_index[i][0].as_in_context(mx.cpu())] = -nd.log(nd.maximum(self.local_name['part_out_data_%s'%i][gt_index[i][0], gt_index[i][1]], 1e-7))[:]
                else:
                    self.batch_loss_mix[gt_index[i][0].as_in_context(mx.cpu())] = -nd.log(nd.maximum(self.local_name['part_out_data_%s'%i][gt_index[i][0], gt_index[i][1]], 1e-7))[:]
    ...
    def loss_forward(self, max_exp, sum_exp, loop=0):
        if self.use_mixup and loop == 1:
            for i in range(self.num_parts):
                self.local_name['part_fully_connected_out_mix_%s'%i][:] = self.local_name['part_fully_connected_out_%s'%i][:]
            if self.margin:
                self.margin_forward(self.local_name['fully_connected_out_mix'], self.margin_s, self.margin_m, self.batch_label[loop])
            self.softmax_forward(self.local_name['fully_connected_out_mix'])
        else:
            cur_label = self.batch_label[0] if self.use_mixup else self.batch_label
            if self.margin:
                self.margin_forward(self.local_name['fully_connected_out'], self.margin_s, self.margin_m, cur_label)
            self.softmax_forward(self.local_name['fully_connected_out'])

        max_exp[:] = self.total_max[:]
        sum_exp[:] = self.total_sum[:]

    def loss_backward(self, total_max_exp, total_sum_exp, loop=0):
        if self.use_mixup and loop == 1:
            self.softmax_backward(self.local_name['fully_connected_out_mix'], total_sum_exp, total_max_exp, self.batch_label[loop], loop)
            if self.margin:
                self.margin_backward(self.local_name['fully_connected_out_mix'], self.margin_s, self.margin_m, self.local_name['softmax_grad_mix'], self.batch_label[loop])
        else:
            cur_label = self.batch_label[0] if self.use_mixup else self.batch_label
            self.softmax_backward(self.local_name['fully_connected_out'], total_sum_exp, total_max_exp, cur_label)
            if self.margin:
                self.margin_backward(self.local_name['fully_connected_out'], self.margin_s, self.margin_m, self.local_name['softmax_grad'], cur_label)

    def fc_backward(self, lamda):
        if self.use_mixup:
            # combine grad by mixup
            for i in range(self.num_parts):
                self.local_name['part_grad_%s'%i][:] = (lamda * self.local_name['part_grad_%s'%i]
                                                     + (1-lamda) * self.local_name['part_grad_mix_%s'%i])[:]
    ...
    def _get_gt_index(self, gt_label):
        gt_index = []
        ...
        for i in range(self.num_parts):
            ...
            self.local_name['part_label_%s'%i][:] = ((gt_label >= part_index_start)*(gt_label<part_index_end)*(gt_label+1))[:]
    ...
    def _margin_func(self, in_data, margin_s, margin_m):
        ...
        if self.easy_margin:
            ...
            zy_keep = in_data - margin_s * mm
        out_data = nd.where(cond, out_data, zy_keep)
        return out_data
    ...
    def output_loss(self, batch_loss, islice, lamda):
        if self.use_mixup:
            self.batch_loss[:] = (lamda * self.batch_loss + (1-lamda) * self.batch_loss)[:]
