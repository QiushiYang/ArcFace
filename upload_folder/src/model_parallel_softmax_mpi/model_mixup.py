...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
                        ...
                        fixed_param_names=[],
                        split_num=None, use_mixup=False,
    ...
    softmax_executor = machine_parallel_loss.SoftmaxModelParallelOperator(train_data.batch_size*num_workers/split_parts,
                                                                          feature_shape, ctx, comm, device_part_size,
                                                                          updater_fc, use_mixup,
                                                                          no_bias, margin, easy_margin, margin_s, margin_m)
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                if use_mixup:
                    eval_total_label = nd.empty((batch_size*num_workers, 2))
                else:
                ...
                for index,islice in enumerate(split_slices):
                    ...
                    if use_mixup:
                        total_label = nd.empty((slice_batch_size*num_workers, 2))
                        if update_on_kvstore:
                    ...
                    softmax_executor.fc_forward()

                    loss_loop = 2 if use_mixup else 1
                    for loop in range(loss_loop):
                        softmax_executor.loss_forward(max_exp, sum_exp, loop)
                        if update_on_kvstore:
                            mpi_gather_bcast(max_exp, sum_exp, max_exp, sum_exp)
                        softmax_executor.loss_backward(max_exp, sum_exp, loop)

                    if nbatch%100==0:
                        batch_loss=nd.zeros(batch_size*num_workers)
                        softmax_executor.output_loss(batch_loss,slice(islice.start*num_workers, islice.stop*num_workers))
                        logging.info('softmaxloss in batch %d: %f'%(nbatch,batch_loss.mean().asnumpy()))
                    softmax_executor.fc_backward(lamda)
                    ...
                    softmax_executor.output_loss(batch_loss, worker_slice, lamda)
                    if update_on_kvstore:
        ...
        if epoch_end_callback or epoch + 1 == end_epoch:
            ...
            if update_on_kvstore:
                ...
                fullyconnect_bias_tmp = nd.empty(fullyconnect_params['weight'].shape)
                cur_rank = nd.array([mpi.rank, kvstore.rank])
                all_rank = nd.empty((comm_size, 2))

                mpi_allgather(weight_tmp, fullyconnect_weight_tmp)
                mpi_allgather(bias_tmp, fullyconnect_weight_tmp)
...
class FeedForward(BASE_ESTIMATOR):
    ...
    def _init_params(self, inputs, num_classes, embedding_size, overwrite=False):
        ...
        for k, v in fullyconnect_params.items():
            if self.fullyconnect_params:
                ...
                self.initializer_fc('weight', fullyconnect_params['weight'])
                self.initializer_fc('bias', fullyconnect_params['bias'])
    ...
    def fit(self, X, y=None, train_metric='acc', eval_data=None, eval_metric='acc',fixed_param_names=[],
            ...
            margin=False, easy_margin=True, margin_s=64, margin_m=0.5,
