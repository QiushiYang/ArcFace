...
def load_property(data_dir):
  prop = edict()
  #for line in open(os.path.join(data_dir, 'property')):
  with fs.reader(os.path.join(data_dir,'property'),True) as r:
...
# def get_dataset_webfafce(input_dir):
...
def get_dataset_webface(input_dir):
  ...
  ret = []
  #for line in open(clean_list_file, 'r'):
  with fs.reader(clean_list_file,True) as r:
    for line in iter(r.readline, ''):
      vec = line.strip().split()
      assert len(vec)==2
      fimage = edict()
...
def _get_dataset_celeb(input_dir):
  ...
  ret = []
  #for line in open(list_file, 'r'):
  with fs.reader(list_file, True) as r:
    for line in iter(r.readline, ''):
      vec = line.strip().split()
      assert len(vec)==2
      fimage = edict()
