...
def parse_args():
  ...
  parser.add_argument('--mean',type=str, default='127.5,127.5,127.5', help='mean value on each channel')
...
def train_net(args):
    ...
    mean = map(float,args.mean.split(','))
    begin_epoch = 0
    base_lr = args.lr
    base_wd = args.wd
    base_mom = args.mom
    ...
    def _batch_callback_hobot(param):
        global_step[0]+=1
        ...
        if mbatch>=0 and mbatch%args.verbose==0:
            save_step[0]+=1
            msave = save_step[0]
            verification_hobot.test(model,mbatch,args.batch_size)
            print('saving', msave)
    ...
    model.fit(train_dataiter,
        ...
        eval_metric        = eval_metrics,    #[NdcgEval(10), LFWVeri(0.0, 30.0, 300)],
        kvstore            = 'local',     #kv, 'device'
        optimizer          = opt,
        ...
        allow_missing      = True,
        batch_end_callback = _batch_callback_hobot,  #mx.callback.Speedometer(args.batch_size, 100),
        epoch_end_callback = epoch_cb)       #checkpoint
