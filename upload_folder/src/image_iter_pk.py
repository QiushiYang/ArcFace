...
class FaceImageIter(io.DataIter):
    ...
    def next(self):
        ...
        if self.repet_flag:
          if self.cur_repet > 0 and self.cur_repet < self.repet_num:
            self.cur_repet += 1
            return self.last_data
          elif self.cur_repet >= self.repet_num:
            self.cur_repet = 0
            self.last_data = None         
        ...
        if self.repet_flag:
          self.cur_repet += 1
          self.last_data = io.DataBatch([batch_data], [batch_label], batch_size - i)
          return self.last_data
        else:
