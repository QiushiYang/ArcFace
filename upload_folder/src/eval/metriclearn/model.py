...
def _ndarray_split(ndarray, num_workers):
    part_size = ndarray.shape[0] / num_workers
    assert part_size*num_workers == ndarray.shape[0]
    return [ndarray[slice(part_size*i, part_size*(i+1))] for i in range(num_workers)]

def mpi_allgather(input_nd, output_nd):
    assert input_nd.size*comm_size == output_nd.size
    sendbuf = input_nd.asnumpy() #nd xpu -> np cpu
    recvbuf = np.empty([comm_size]+list(sendbuf.shape), dtype=sendbuf.dtype)
    comm.Allgather(sendbuf, recvbuf)
    output_nd[:] = recvbuf.reshape(output_nd.shape) # np cpu -> nd xpu

def mpi_reduce_scatter(input_nd, output_nd):
    assert input_nd.size == output_nd.size*comm_size
    sendbuf = input_nd.asnumpy() #nd xpu -> np cpu
    allbuf = np.empty(sendbuf.shape, dtype=sendbuf.dtype) if comm_rank==0 else None
    comm.Reduce(sendbuf, allbuf, root=0, op=MPI.SUM)
    recvbuf = np.empty(output_nd.shape, sendbuf.dtype)
    comm.Scatter(allbuf.reshape([comm_size]+list(recvbuf.shape)) if comm_rank==0 else None, recvbuf, root=0)
    output_nd[:] = recvbuf # np cpu -> nd xpu
    

# model
Executor = namedtuple('Executor', ['executor', 'datas', 'data_grads'])

def MetricExecutor(symbol, input_shape, num_workers, ctx, ):
    batch_size = input_shape[0]
    qry = nd.zeros(input_shape, ctx=ctx)
    qry_grad = nd.zeros(input_shape, ctx=ctx)
    qry_label = nd.zeros((batch_size,), ctx=ctx)
    ref_shape = tuple([batch_size*num_workers]+list(input_shape[1:]))
    ref = nd.zeros(ref_shape, ctx=ctx)
    ref_grad = nd.zeros(ref_shape, ctx=ctx)
    ref_label = nd.zeros((ref_shape[0],), ctx=ctx)
    args = {"qry" : qry, "qry_label" : qry_label, "ref" : ref, "ref_label" : ref_label}
    grads = {"qry" : qry_grad, "ref" : ref_grad}
    reqs = {"qry" : "write", "ref" : "write"}
    executor = symbol.bind(ctx=ctx, args=args, args_grad=grads, grad_req=reqs)
    return Executor(executor=executor, datas=args, data_grads=grads)
...
def _train_multi_device(symbol, metric_symbol, ctx, arg_names, param_names, aux_names,
                        arg_params, aux_params,
                        begin_epoch, end_epoch, epoch_size, optimizer,
                        kvstore, update_on_kvstore,
                        train_data, split_num=None, eval_data=None, 
                        train_metric=None, eval_metric=None,
                        fixed_param_names=[],
                        epoch_end_callback=None, batch_end_callback=None,
                        logger=None, work_load_list=None, monitor=None,
                        eval_batch_end_callback=None, eval_first=False,
                        l2_reg=0.0, svd_epoches_period=0, svd_param_name=None):
    ...
    print("trainable params: {}".format(trainable_param_names))
    ...
    metric_executor = MetricExecutor(
        ...
        input_shape=tuple([train_data.batch_size]+list(feature_shape)), 
        num_workers=kvstore.num_workers if update_on_kvstore else 1, 
        ctx=ctx[0] if isinstance(ctx, list) else ctx,#metric_cost do not support multi-gpu
    )

    # split data_batch to small batches
    num_parts = 1 if split_num is None else split_num
    part_size = int(train_data.batch_size / num_parts)
    assert part_size*num_parts == train_data.batch_size
    data_slices = [slice(i*part_size, (i+1)*part_size) for i in range(num_parts)]

    # Now start training
    train_data.reset()
    eval_datas = eval_data if isinstance(eval_data, list) else [] if eval_data is None else [eval_data]
    if eval_first:
        _eval_data(eval_datas, eval_metric, part_size, executor_manager, eval_batch_end_callback, begin_epoch - 1,
                   logger)
    for epoch in range(begin_epoch, end_epoch):
        ...
        while True:
            ...
            for data_batch in train_data:
                ...
                if monitor is not None:
                    monitor.tic()
                # forward only to get qry
                cpu_out_feature = nd.zeros(metric_executor.datas["qry"].shape)
                if len(data_slices)>1:
                    for islice in data_slices:
                        executor_manager.load_data_batch(data_batch, islice)
                        executor_manager.forward(is_train=False)
                        executor_manager.output_copyto(cpu_out_feature[islice])
                else:                   
                    for islice in data_slices:
                        executor_manager.load_data_batch(data_batch, islice)
                ...
                # allgather ref label/feature
                ref = metric_executor.datas["ref"]
                ref_label = metric_executor.datas["ref_label"]
                if update_on_kvstore:
                    mpi_allgather(cpu_out_feature, ref)
                    mpi_allgather(data_batch.label[0], ref_label)
                else:
                    cpu_out_feature.copyto(ref)
                    data_batch.label[0].copyto(ref_label)

                # metric cost
                qry = metric_executor.datas["qry"]
                qry_label = metric_executor.datas["qry_label"]
                cpu_out_feature.copyto(qry)
                data_batch.label[0].copyto(qry_label)
                metric_executor.executor.forward(is_train=True)
                metric_executor.executor.backward()

                # assert np.allclose(data_batch.label[0].asnumpy(),
                #                    ref_label.asnumpy())
                # assert np.allclose(cpu_out_feature.asnumpy()
                #                    ref.asnumpy())

                # reduce scatter ref_grad
                if update_on_kvstore:
                    ref_grad = nd.zeros(qry.shape, ctx=qry.context)
                    mpi_reduce_scatter(metric_executor.data_grads["ref"], ref_grad)
                    #assert np.allclose(metric_executor.data_grads["ref"].asnumpy(),
                    #                   ref_grad.asnumpy())
                else:
                    ref_grad = metric_executor.data_grads["ref"]
            
                # gradient
                qry_grad = metric_executor.data_grads["qry"]
                out_grad = qry_grad + ref_grad

                if l2_reg > 0:
                    if tmp_counter % 100 == 0 and comm_rank == 0:
                        print 'l2_grad is ' + str(l2_reg * qry.asnumpy())
                        print 'l2_abs grad is ' + str(np.mean(np.abs(l2_reg * qry.asnumpy())))
                        print 'out_grad is ' + str(out_grad.asnumpy())
                    tmp_counter += 1
                    out_grad = out_grad + l2_reg * qry
                ...
                if len(data_slices)>1:
                    ...
                    for islice in data_slices:
                        executor_manager.load_data_batch(data_batch, islice)
                        executor_manager.forward(is_train=True)
                        executor_manager.backward(out_grad[islice])
                else:
                ...
                train_metric.update(data_batch.label, metric_executor.executor.outputs)
        ...
        if epoch_end_callback or epoch + 1 == end_epoch:
            ...
            if svd_epoches_period > 0 and epoch % svd_epoches_period == 0:
                logger.info("Doing SVD Decomposition")
                arg_params[svd_param_name] = do_svd(arg_params[svd_param_name])
                executor_manager.set_params(arg_params, aux_params)
        ...
        _eval_data(eval_datas, eval_metric, part_size, executor_manager, eval_batch_end_callback, epoch, logger)
    # end of all epochs
    return
...
class FeedForward(BASE_ESTIMATOR):
    ...
    """
    def __init__(self, symbol, metric_symbol=None, ctx=None,
        ...
        self.symbol = symbol
        self.metric_symbol = metric_symbol
    ...
    def fit(self, X, y=None, split_num=None, eval_data=None, 
            train_metric='acc', eval_metric='acc', fixed_param_names=[],
            epoch_end_callback=None, batch_end_callback=None, kvstore='local', logger=None,
            work_load_list=None, monitor=None, eval_batch_end_callback=None, eval_first=False,
            l2_reg=0.0, svd_epoches_period=0, svd_param_name=None):
        """Fit the model.
        ...
        # do training
        _train_multi_device(self.symbol, self.metric_symbol, self.ctx, 
                            arg_names, param_names, aux_names,
                            ...
                            eval_batch_end_callback=eval_batch_end_callback, eval_first=eval_first,
                            l2_reg=l2_reg, svd_epoches_period=svd_epoches_period, svd_param_name=svd_param_name)
    ...
    def create(symbol, X, y=None, ctx=None,
        ...
        model.fit(X, y, eval_data=eval_data, train_metric=eval_metric, eval_metric=eval_metric,
                  epoch_end_callback=epoch_end_callback,
                  batch_end_callback=batch_end_callback,
                  kvstore=kvstore,
                  logger=logger,
