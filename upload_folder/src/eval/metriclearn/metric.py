...
def LCosDis(qry, ref):
    #pdb.set_trace()
    dist = LDOT(qry, ref.T)
    qry_ss = np.sqrt((qry**2).sum(axis=1).reshape(qry.shape[0], 1))
    ref_ss = np.sqrt((ref**2).sum(axis=1).reshape(1, ref.shape[0]))
    dist = dist/qry_ss
    dist = dist/ref_ss
    dist = np.arccos(dist)
    return dist
...
def CalSoftMinAccuracy_MPI(local_qry_feat, local_qry_label, ref_feat, ref_label, gamma_list, top_hit, dist=None,
                           fid=None):
    # compute dist
    if dist is None:
    ...
    if dl.max() != 1 or dl.min() < 0 or ref_label[0] != 0 or ref_cate_num != dl.sum() + 1:
        print('{}, {}, {}, {}, {}'.format(dl.max(), dl.min(), ref_label[0], ref_cate_num, dl.sum() + 1))
    ...
    for gamma in gamma_list:
        ...
        log = "{:.3G}-MIN, {}x{}: avg-dist {:.4G} max-dist {:.4G}\n\t".format(gamma, qry_num, ref_label.size, avg_dist,
                                                                              max_dist)
...
def CalTheKthNNAccuracy_MPI(local_qry_feat, local_qry_label, ref_feat, ref_label, k_list, top_hit, dist=None, fid=None):
    ...
    for k in k_list:
        ...
        log = "{}th-MIN, {}x{}: avg-dist {:.4G} max-dist {:.4G}\n\t".format(k, qry_num, ref_label.size, avg_dist,
                                                                            max_dist)
        for idx, hit in enumerate(top_hit_rate):
            log += '{}:{:.5G} '.format(idx, hit)
        # log += '\n\t'
...
def CalAccuracyTopN_MPI(local_qry_coord, local_qry_label, ref_coord, ref_label, topN_l, gamma_l=None, mAP_NDCG=True,
    ...
    for topN in topN_l:
        ...
        log += 'Top-{}: accuracy {:.4f}, hit rate {:.4f}, recall rate {:.4f}\n'.format(topN,
                                                                                       float(correct_num) / (QN * topN),
                                                                                       float(hit_num) / QN,
                                                                                       float(recall_rate_sum) / QN)
    if comm_rank == 0:
        print(log)
        if fid is not None:
            fid.write(log + '\n'),
    ...
    if gamma_l is not None:
        ...
        for topN in topN_l:
            ...
            local_err_num_gamma = np.vstack(
                [0.5 * ((local_hit_rank[:, :, 0] > k).sum(axis=0) + (local_hit_rank[:, :, 1] > k).sum(axis=0)) for k in
                 range(K)])
...
def CalSoftMaxErrorMultiPrototype_MPI(x, y, cate_num, prototype_per_cate, topK_l, fid=None):
    ...
    log = 'SoftMaxError: cate_num {}, prototype_per_cate {}, total_smp_num {}; dist {:.5f}+/{:.5f}, {:.5f}~{:.5f}\n'.format(
        cate_num, prototype_per_cate, total_num, dx.mean(), dx.std(), dx.min(), dx.max())
    for topK in topK_l:
...
def CalDistSTD_MPI(local_dist, fid=None):
    ...
    log = 'Distance: {}x{}, {:.4f}+-{:.4f}, min {:.4f}, max{:.4f}'.format(qry_num, local_dist.shape[1], avg, std, min_,
                                                                          max_)
    if comm_rank == 0:
        print(log)
    if fid is not None:
...
def InterpolateROC(ROC, false_positive=None, false_negative=None):
    if false_positive is not None:
        ...
        assert false_negative is not None
...
def CalROC_MPI(local_pos_conf, local_neg_conf, interesting_false_positive_l=[], interesting_false_negative_l=[],
               bins=10000, delta_thres=0.01):
    ...
    items = comm.reduce(np.array(
        [local_pos_num, local_pos_conf_sum, local_pos_conf_sum2, local_neg_num, local_neg_conf_sum,
         local_neg_conf_sum2], dtype='float'), root=0, op=MPI.SUM)
    info = ''
    if comm_rank == 0:
...
def CalROC_GivenCoord_MPI(local_tst_coord, local_tst_label, interesting_false_positive_l=[],
                          interesting_false_negative_l=[], bins=10000, delta_thres=0.01):
    tst_coord = my_Allgatherv(local_tst_coord)
    tst_label = my_Allgatherv(local_tst_label)
    ...
    return CalROC_MPI(local_pos_conf, local_neg_conf, interesting_false_positive_l, interesting_false_negative_l, bins,
                      delta_thres)


def GetClusterCentroids_MPI(local_coord, local_cate_offset, cluster_num_per_cate, iter_num=1, show_info=True,
                            gather=True):
    ...
    for label in sorted(local_cate_offset.keys()):
        ...
        if show_info:
            print('Rank {}/{}: cate {}/{} is done'.format(comm_rank, comm_size, len(local_centroids_l),
                                                          len(local_cate_offset)))
...
def findMetricThreshold_MPI(local_qry_feat, local_qry_label, local_ref_feat, local_ref_label, dist=None, fid=None):
    ...
    for lqi in range(local_qry_num):
        ...
        #tmp_indice = np.where(intra_indices != lqi)
        #intra_indices = intra_indices[tmp_indice]
...
global_dist = None
def thread_func(local_qry_num, ref_label, local_qry_label,
                beg, interval):
    global global_dist
    dist = global_dist
    intra_v = []
    ...
    return intra_v, inter_v, intra_num, inter_num

# added by degang.yang
#from mxnet_face/plugin/metriclearn/python/metriclearn/metric.py
## Multi-process Version
def findMetricThreshold_Mulpro(local_qry_feat, local_qry_label, ref_feat, ref_label, dist=None, fid=None):
    if dist is None:
        dist = LEculidean(local_qry_feat, ref_feat)
    else:
        assert dist.shape == (local_qry_feat.shape[0], ref_feat.shape[0])
    ...
    for i in range(slice_num):
        results_list.append( \
            pool_handler.apply_async(thread_func, (local_qry_num, ref_label, local_qry_label,  \
                i, slice_num)))
    pool_handler.close()
    pool_handler.join()
...
def findMetricThreshold(local_qry_feat, local_qry_label, ref_feat, ref_label, dist=None, fid=None):
    if dist is None:
        dist = LEculidean(local_qry_feat, ref_feat)
        # dist = LCosDis(local_qry_feat, ref_feat)
    else:
        assert dist.shape == (local_qry_feat.shape[0], ref_feat.shape[0])
    #dist[dist <= 0] = 0.0
    #dist = np.sqrt(dist)
    ...
    inter_v = []
    for lqi in range(local_qry_num):
        tmp_label = local_qry_label[lqi]
        ref_label[lqi] = 2000000
    ...
    logging.info('Intra Distance: {}, {:.4f}+-{:.4f}, min {:.4f}, max {:.4f}'.format(intra_num, intra_avg, intra_std,
                                                                              intra_min, intra_max))
    logging.info('Inter Distance: {}, {:.4f}+-{:.4f}, min {:.4f}, max {:.4f}'.format(inter_num, inter_avg, inter_std,
                                                                              inter_min, inter_max))
    ...
    #inter_v.sort()
    FAR = [0.01, 0.001, 0.0001, 0.00001, 0.000001]
    ...
    for k in range(len(FAR)):
        ...
        cnt = len(intra_v[intra_v < thr])
        GAR = float(cnt) / intra_num
        logging.info("thr:%.4f  FAR:%.5f(%d/%d)  GAR:%.5f(%d/%d)" % (thr, FAR[k], num, inter_num, GAR, cnt, intra_num))
