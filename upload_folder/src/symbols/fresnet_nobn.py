...
def residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    if bottle_neck:
        ...
        if dim_match:
            ...
            shortcut = Conv(data=data, num_filter=num_filter, kernel=(1,1), stride=stride, no_bias=True,
        ...
        out_put=Act(data=conv3 + shortcut, act_type=act_type, name=name + '_relu3')
        if mirroring_level >= 1:
        ...
        out_put = Act(data=conv2 + shortcut, act_type=act_type, name=name + '_relu3')
        if mirroring_level >= 1:
            out_put._set_attr(force_mirroring='True')
        return out_put
...
def residual_unit_v2(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    if bottle_neck:
        ...
        #                        if mirroring_level >= 2 else {},
        #                        name=name + '_bn1')
        act1 = Act(data=data, act_type=act_type, name=name + '_relu1')
        if mirroring_level >= 1:
            act1._set_attr(force_mirroring='True')
        ...
        #                        if mirroring_level >= 2 else {},
        #                        name=name + '_bn2')
        act2 = Act(data=conv1, act_type=act_type, name=name + '_relu2')
        if mirroring_level >= 1:
            act2._set_attr(force_mirroring='True')
        ...
        #                        if mirroring_level >= 2 else {},
        #                        name=name + '_bn3')
        act3 = Act(data=conv2, act_type=act_type, name=name + '_relu3')
        if mirroring_level >= 1:
...
def residual_unit_v3_x(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    #                        if mirroring_level >= 2 else {},
    #                        name=name + '_bn1')
    conv1 = Conv(data=data, num_group=num_group, num_filter=int(num_filter*0.5), kernel=(1,1), stride=(1,1), pad=(0,0),
                               no_bias=True, workspace=workspace, name=name + '_conv1')
    # bn2 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=bn_mom,
    #                        sync=True if use_sync_bn else False,
    #                        attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck, **kwargs):
    ...
    #fc1 = symbol_utils_nobn.get_fc1(body, num_classes, fc_type,**kwargs)
    fc1 = symbol_utils_nobn.get_fc1(body, num_classes, fc_type,**kwargs)
    return fc1
