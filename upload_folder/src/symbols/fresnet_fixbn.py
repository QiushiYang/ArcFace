...
def residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    use_sync_bn = kwargs.get('use_sync_bn', 0)
    mirroring_level = kwargs.get('mirroring_level', 0)
    #print('in unit1')
    if bottle_neck:
        ...
        conv2 = Conv(data=act1, num_filter=int(num_filter*0.25), kernel=(3,3), stride=stride, pad=(1,1),
        ...
        if dim_match:
            ...
            shortcut = mx.sym.BatchNorm(data=conv1sc, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                                        use_global_stats=True,
                                        sync=True if use_sync_bn else False,
                                        attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                                        if mirroring_level >= 2 else {},
        ...
        out_put=Act(data=bn3 + shortcut, act_type=act_type, name=name + '_relu3')
        if mirroring_level >= 1:
            out_put._set_attr(force_mirroring='True')
        return out_put
        ...
        if dim_match:
            ...
            conv1sc = Conv(data=data, num_filter=num_filter, kernel=(1,1), stride=stride, no_bias=True,
                                            workspace=workspace, name=name+'_conv1sc')
            shortcut = mx.sym.BatchNorm(data=conv1sc, fix_gamma=False, momentum=bn_mom, eps=2e-5,
                                        use_global_stats=True,
                                        sync=True if use_sync_bn else False,
                                        attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                                        if mirroring_level >= 2 else {},
        ...
        out_put = Act(data=bn2 + shortcut, act_type=act_type, name=name + '_relu3')
        if mirroring_level >= 1:
            out_put._set_attr(force_mirroring='True')
        return out_put
...
def residual_unit_v2(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    if bottle_neck:
        ...
        if mirroring_level >= 1:
            act3._set_attr(force_mirroring='True')
...
def residual_unit_v3(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    if bottle_neck:
        ...
        conv3 = Conv(data=act2, num_filter=num_filter, kernel=(1,1), stride=stride, pad=(0,0), no_bias=True,
                                   workspace=workspace, name=name + '_conv3')
        bn4 = mx.sym.BatchNorm(data=conv3, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                               use_global_stats=True,
                               sync=True if use_sync_bn else False,
                               attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                               if mirroring_level >= 2 else {},
        ...
        if dim_match:
            ...
            conv1sc = Conv(data=data, num_filter=num_filter, kernel=(3,3) if stride[0]==2 and stride[1]==2 else (1,1), stride=stride,
                                            pad=(1, 1) if stride[0]==2 and stride[1]==2 else (0,0), no_bias=True,
                                            workspace=workspace, name=name+'_conv1sc')
            shortcut = mx.sym.BatchNorm(data=conv1sc, fix_gamma=False, momentum=bn_mom, eps=2e-5,
                                        use_global_stats=True,
                                        sync=True if use_sync_bn else False,
...
def residual_unit_v4(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    act_type = kwargs.get('version_act', 'prelu')
    use_sync_bn = kwargs.get('use_sync_bn', 0)
    num_group = kwargs.get('num_group', 32)
    mirroring_level = kwargs.get('mirroring_level', 0)
    #num_group=64
    a = 0.5
    if  num_group==64:
    ...
    conv1 = Conv(data=data, num_filter=int(num_filter*a), kernel=(1,1), stride=(1,1), pad=(0,0),
                                   no_bias=True, workspace=workspace, name=name + '_conv1')
    bn1 = mx.sym.BatchNorm(data=conv1, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                           use_global_stats=True,
                           sync=True if use_sync_bn else False,
    ...
    conv2 = Conv(data=act1, num_filter=int(num_filter*a), kernel=(3,3), stride=stride, pad=(1,1),
                     num_group=num_group,no_bias=True, workspace=workspace, name=name + '_conv2')
    bn2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                           use_global_stats=True,
                           sync=True if use_sync_bn else False,
                           attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
...
def residual_unit_v4_L(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    """Return ResNet Unit symbol for building ResNet
    Parameters
    ----------
    data : str
    ...
    if mirroring_level >= 1:
        act1._set_attr(force_mirroring='True')
    conv2 = Conv(data=act1, num_filter=int(num_filter*a), kernel=(3,3), stride=(1,1), pad=(1,1),
                 num_group=num_group,no_bias=True, workspace=workspace, name=name + '_conv2')
    bn2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                           use_global_stats=True,
                           sync=True if use_sync_bn else False,
...
def AttentionModule_stage2(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    body = residual_unit(data, num_filter, stride, True, name=name+'r1',
            bottle_neck=bottle_neck, **kwargs)
    out_trunk = residual_unit(body, num_filter, stride, True, name=name+'r2',
            bottle_neck=bottle_neck, **kwargs)
    out_trunk = residual_unit(out_trunk, num_filter, stride, True, name=name+'r3',
            bottle_neck=bottle_neck, **kwargs)
    pool1 = mx.sym.Pooling(data=body, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')
    mask1 = residual_unit(pool1, num_filter, stride, True, name=name+'m1',
            bottle_neck=bottle_neck, **kwargs)
    skip1 = residual_unit(mask1, num_filter, stride, True, name=name+'s1',
            bottle_neck=bottle_neck, **kwargs)
    pool2 = mx.sym.Pooling(data=mask1, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')
    mask2 = residual_unit(pool2, num_filter, stride, True, name=name+'m2_1',
            bottle_neck=bottle_neck, **kwargs)
    mask2 = residual_unit(mask2, num_filter, stride, True, name=name+'m2_2',
            bottle_neck=bottle_neck, **kwargs)
    interp2 = mx.sym.UpSampling(data=mask2, sample_type='bilinear', scale = 2, num_filter=num_filter, workspace=workspace, name=name+'interp2') + mask1
    ...
    out = interp2 + skip1
    mask3 = residual_unit(out, num_filter, stride, True, name=name+'m3',
            bottle_neck=bottle_neck, **kwargs)
    interp1 = mx.sym.UpSampling(data=mask3, sample_type='bilinear', scale = 2, num_filter=num_filter, workspace=workspace, name=name+'interp1') + out_trunk
    ...
    mask4 = mx.sym.Convolution(data=mask4, num_filter=num_filter, kernel=(1,1), stride=(1,1), pad=(0,0),
    ...
    out_last = residual_unit(out, num_filter, stride, True, name=name+'last',
            bottle_neck=bottle_neck, **kwargs)
    return out_last
...
def AttentionModule_stage1(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    out_trunk = residual_unit(out_trunk, num_filter, stride, True, name=name+'r3',
            bottle_neck=bottle_neck, **kwargs)
    pool1 = mx.sym.Pooling(data=body, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')
    mask1 = residual_unit(pool1, num_filter, stride, True, name=name+'m1_1',
            bottle_neck=bottle_neck, **kwargs)
    mask1 = residual_unit(mask1, num_filter, stride, True, name=name+'m1_2',
            bottle_neck=bottle_neck, **kwargs)
    interp1 = mx.sym.UpSampling(data=mask1, sample_type='bilinear', scale = 2, num_filter=num_filter, workspace=workspace, name=name+'interp1') + out_trunk
    #interp1 = mx.sym.Deconvolution(data=mask3, num_filter=output_channels, kernel=(4,4), stride=(2,2), pad=(1,1), num_group=1,#output_channels,
    #                                  no_bias=True, workspace=256, name=name + 'interp1') + out_trunk
    mask4 = mx.sym.BatchNorm(data=interp1, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                             use_global_stats=True,
...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
  ...
  if uv==1:
    ...
    if version_input <=1:
      ...
      return residual_unit_v4_L(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs)
  else:
    if version_input<=1:
...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck, **kwargs):
    ...
    #data = mx.symbol.Custom(data, layername='after data', op_type='ProbeLayer')
    data = data-127.5
    data = data*0.0078125
    ...
    if version_input==0:
      ...
      body = Conv(data=body, num_filter=filter_list[0], kernel=(3,3), stride=(1,1), pad=(1, 1),
                                no_bias=True, name="conv0", workspace=workspace)
      #body = mx.symbol.Custom(body, layername='after conv0', op_type='ProbeLayer')
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                              use_global_stats=True,
                              sync=True if use_sync_bn else False,
                              attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
      ...
      body = Conv(data=body, num_filter=filter_list[0], kernel=(3,3), stride=(1,1), pad=(1, 1),
                                no_bias=True, name="conv0-1", workspace=workspace)
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom,
                              use_global_stats=True,
                              sync=True if use_sync_bn else False,
                              attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                              if mirroring_level >= 2 else {},
                              name='bn0-1')
      body = Act(data=body, act_type=act_type, name='relu0-1')
      if mirroring_level >= 1:
          body._set_attr(force_mirroring='True')
      #body = mx.sym.Pooling(data=body, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')
    ...
    for i in range(num_stages):
      ...
      if i == 1:
        if version_unit == 5:
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 2),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 3),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 4),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage2(body, filter_list[i+1], (1,1), True, name='a2_1',
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage2(body, filter_list[i+1], (1,1), True, name='a2_2',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 5),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 6),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage2(body, filter_list[i+1], (1,1), True, name='a2_3',
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage2(body, filter_list[i+1], (1,1), True, name='a2_4',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 7),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 8),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage2(body, filter_list[i+1], (1,1), True, name='a2_5',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 9),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 10),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 11),
            bottle_neck=bottle_neck, **kwargs)
        else:
        ...
        if version_unit == 8:
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 2),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_1',
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_2',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 3),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_3',
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_4',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 4),
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_5',
            bottle_neck=bottle_neck, **kwargs)
          body = AttentionModule_stage1(body, filter_list[i+1], (1,1), True, name='a1_6',
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 5),
            bottle_neck=bottle_neck, **kwargs)
          body = residual_unit(body, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i+1, 6),
    ...
    fc1 = symbol_utils_fixbn.get_fc1(body, num_classes, fc_type,**kwargs)
    return fc1
