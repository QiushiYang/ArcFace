...
def get_fc1(last_conv, num_classes, fc_type, **kwargs):
  #mirroring_level = kwargs.get('mirroring_level', 0)
  use_sync_bn = kwargs.get('use_sync_bn', 0)
  bn_mom = 0.9
  body = last_conv
  if fc_type=='Z':
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, use_global_stats=True, eps=2e-5, momentum=bn_mom,
    ...
    fc1 = mx.sym.BatchNorm(data=fc1, fix_gamma=True, use_global_stats=True, eps=2e-5, momentum=bn_mom,
                           sync=True if use_sync_bn else False,
                           #attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                           #if mirroring_level >= 2 else {},
                           name='fc1')
    ...
    bn1 = mx.sym.BatchNorm(data=body, fix_gamma=False, use_global_stats=True, eps=2e-5, momentum=bn_mom,
                           sync=True if use_sync_bn else False,
                           #attr={'force_mirroring': 'True', 'cudnn_off': 'True'}
                           #if mirroring_level >= 2 else {},
                           name='bn1')
    ...
    if fc_type=='A':
      ...
      if fc_type=='C':
        ...
        #if mirroring_level >= 1:
        #    fc1._set_attr(force_mirroring='True')
  return fc1
