...
def _train_multi_device(symbol, ctx, arg_names, param_names, aux_names,
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        if epoch >= 1 and epoch % 5 == 0:
            train_data.reset()
            seq_index = np.empty((train_data.num_samples()))
            seq_loss = np.empty((train_data.num_samples()))
            idx = 0
            for i, train_batch in enumerate(train_data):
                out_shape = tuple([batch_size] + list(executor_manager.output_shapes[0][1:]))
                cpu_out_feature = nd.zeros(out_shape)
                executor_manager.load_data_batch(train_batch)
                executor_manager.forward(is_train=False)
                executor_manager.output_copyto(cpu_out_feature)

                if update_on_kvstore:
                    total_feature = nd.empty((batch_size*num_workers,)+feature_shape)
                    total_label = nd.empty((batch_size*num_workers,))
                    mpi_allgather(cpu_out_feature, total_feature)
                    mpi_allgather(train_batch.label[0], total_label)
                    softmax_executor.load_data(total_feature, total_label)
                else:
                    softmax_executor.load_data(cpu_out_feature, train_batch.label[0])

                # fully connected and softmax
                max_exp = nd.empty((batch_size*num_workers,))
                sum_exp = nd.empty((batch_size*num_workers,))
                softmax_executor.forward(max_exp, sum_exp)
                if update_on_kvstore:
                    mpi_gather_bcast(max_exp, sum_exp, max_exp, sum_exp)
                ...
                batch_loss = nd.empty((batch_size,))
                softmax_executor.output_loss(batch_loss, slice(0, batch_size))

                total_index = nd.empty((batch_size*num_workers,))
                total_loss = nd.empty((batch_size*num_workers,))
                if update_on_kvstore:
                    mpi_allgather(train_batch.index[0], total_index)
                    mpi_allgather(batch_loss, total_loss)
                    step = train_data.batch_size*num_workers
                    seq_loss[idx: idx+step] = total_loss.asnumpy()
                    seq_index[idx: idx+step] = total_index.asnumpy()
                    idx += step
                else:
                    seq_loss[idx: idx+train_data.batch_size] = batch_loss.asnumpy()
                    seq_index[idx: idx+train_data.batch_size] = train_batch.index[0].asnumpy()
                    idx += train_data.batch_size

            unique_index, unique_idx = np.unique(seq_index[0:idx], return_index=True)
            unique_loss = seq_index[unique_idx]
            arg_loss = np.argsort(unique_loss)
            sort_index = unique_index[arg_loss].tolist()
            sort_index.reverse()

            train_data.update_index.put_nowait(sort_index)
    # end of all epochs
    return
